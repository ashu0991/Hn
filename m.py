...
admin_id = ["5728282067","5077667559"]
...
def welcome_start(message):
    response = "Welcome to LAUDA LASHUN"
    try:
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # @PrivateFileTg # Updated to accept target, time, and port
            ...
            response = "✅ Usage :- /bgmi <target> <port> <time> आंडी बांडी सांडी जवन Spam करी ओकर मॉल रण्डी"  # @PrivateFileTg # Updated command syntax
    else:
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        response = "ONLY A S H U SIR  CAN USE."
